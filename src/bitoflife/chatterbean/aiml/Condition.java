...
public class Condition extends TemplateElement {
	...
	public Condition(Attributes attributes) {
		...
		for (int i = 0; i < value.length(); i++) {
			java.util.regex.Pattern p = java.util.regex.Pattern
					.compile("^[\u4E00-\u9FA5|\\*]+$");
			java.util.regex.Matcher m = p.matcher(String.valueOf(value
					.charAt(i)));
			if (m.matches()) {
				temp.append(value.charAt(i)).append(" ");
			} else {
	...
	public String process(Match match) {
		...
		String compared = (String) context.property("predicate." + name, match
				.getUserId());
		compared = transformations.normalization(compared);
		if (value != null) {
			...
			Li li = find(context, transformations, match.getUserId());
			return (li != null ? li.process(match) : "");
		}
	}
